...
eDomoticzAccessory.prototype = {
    ...
    handleMQTTMessage: function (message, callback) {
        ...
        if ((this.Type == "P1 Smart Meter" && this.swTypeVal == 0 && this.subType == "Energy") || (this.Type == "P1 Smart Meter" && this.swTypeVal == 1 && this.subType == "Gas") || (this.Type == "General" && this.swTypeVal == 2 && this.subType == "Counter Incremental") || (this.name.indexOf("Occupied") > -1) || (this.Type == "General" && this.swTypeVal == 1 && this.subType == "Visibility") || (this.Type == "General" && this.swTypeVal === 0 && this.subType == "kWh") || (this.Type == "General" && this.subType == "Solar Radiation" && this.swTypeVal === 0) || (this.Type == "YouLess Meter" && this.swTypeVal === 0) || (this.name.indexOf("Location") > -1)|| (this.name.indexOf("UPS") > -1) || (this.Type == "RFXMeter")) {
                    ...
                    if ('undefined' !== typeof message.svalue1) {
                        if (this.subType == "kWh" || this.subType == "YouLess counter" || this.subType == "Electric") {
                            ...
                            var service = this.getService(Service.HumiditySensor);
                                var BaroDeviceCharacteristic = this.getCharacteristic(service, eDomoticzServices.Barometer);
                                var val15 = Helper.cleanFloat(message.svalue1);
                                callback(BaroDeviceCharacteristic, val15);
                                break;
                         } else if (this.name.indexOf("UPS") > -1) {
                            var service = this.getService(Service.BatteryService);
                            var batServiceCharacteristic = this.getCharacteristic(service, Characteristic.BatteryLevel);
                            callback(batServiceCharacteristic, message.Battery);
                            var batServiceCharacteristic = this.getCharacteristic(service, Characteristic.ChargingState);
                            callback(batServiceCharacteristic, message.nvalue);
                            break;   
                        } else {
                            var service = this.getService(eDomoticzServices.MeterDeviceService);
    ...
    getServices: function () {
        ...
        //console.log("AG >>>>> subType " + this.subType + " isSwitch " + this.isSwitch );
        if (this.isSwitch) {
            switch (true) {
            case this.swTypeVal == Constants.DeviceTypeSwitch || this.swTypeVal == Constants.DeviceTypePushOn:
                {
            ...
            case this.swTypeVal == Constants.DeviceTypeDoorContact:
                {
                    ...
                    if (this.batteryRef && this.batteryRef !== 255) { // if batteryRef == 255 we're running on mains
                            this.gracefullyAddCharacteristic(contactService, Characteristic.StatusLowBattery).on('get', this.getLowBatteryStatus.bind(this));
                            this.gracefullyAddCharacteristic(contactService, Characteristic.BatteryLevel).on('get', this.getBatteryLevel.bind(this));
                    }
            ...
            case this.swTypeVal == Constants.DeviceTypeSmoke:
                {
                    if (this.name.indexOf("Leak") > -1) {
                        ...
                        if (this.batteryRef && this.batteryRef !== 255) { // if batteryRef == 255 we're running on mains
                            ...
                            this.gracefullyAddCharacteristic(leakService, Characteristic.BatteryLevel).on('get', this.getBatteryLevel.bind(this));
                        }
 ...
 //                           this.gracefullyAddCharacteristic(smokeService, Characteristic.BatteryLevel).on('get', this.getBatteryLevel.bind(this));
                        }
                        this.services.push(smokeService);
                        //var batService = this.getService(Service.BatteryService);
                        //if (!batService) {
                        //    batService = new Service.BatteryService(this.name);
                        //}
                        //this.getCharacteristic(batService, Characteristic.BatteryLevel).on('get', this.getBatteryLevel.bind(this));
                        
                        //this.services.push(batService);
            ...
            case this.swTypeVal == Constants.DeviceTypeMedia:
                {
                    var lightbulbService = this.getService(Service.Lightbulb);
                    if (!lightbulbService) {
            ...
            case this.swTypeVal == Constants.DeviceTypeSelector:
                {
                    ...
                    if (this.batteryRef && this.batteryRef !== 255) { // if batteryRef == 255 we're running on mains
                        ...
                        this.gracefullyAddCharacteristic(selectorService, Characteristic.BatteryLevel).on('get', this.getBatteryLevel.bind(this));
                    }
            ...
            case this.swTypeVal == Constants.DeviceTypeMotion:
                {
                    ...
                    if (this.batteryRef && this.batteryRef !== 255) { // if batteryRef == 255 we're running on mains
                        ...
                        this.gracefullyAddCharacteristic(motionService, Characteristic.BatteryLevel).on('get', this.getBatteryLevel.bind(this));
                    }
            ...
            case this.Type == "General" || this.Type == "RFXMeter" || this.Type == "YouLess Meter" || this.Type == "Current" || this.Type == "UV" || this.Type == "Usage" || this.Type == "Lux":
                {
                    if (this.subType == "kWh" || this.subType == "YouLess counter" || this.subType == "Electric") {
                        ...
                        var BaroDeviceService = this.getService(Service.HumiditySensor);
                        if (!BaroDeviceService) {
                            BaroDeviceService = new Service.HumiditySensor(this.name);
                        }
                        //this.getCharacteristic(BaroDeviceService, eDomoticzServices.Volt).on('get', this.getStringValue.bind(this));
                        this.gracefullyAddCharacteristic(BaroDeviceService, eDomoticzServices.Barometer).on('get', this.getPressure.bind(this));
                        this.services.push(BaroDeviceService);
                        break;
                     } else if (this.name.indexOf("UPS") > -1) {
                         var batService = this.getService(Service.BatteryService);
                        if (!batService) {
                        ...
                        batService.setCharacteristic(Characteristic.ChargingState, 1)
                        this.getCharacteristic(batService, Characteristic.BatteryLevel).on('get', this.getBatteryLevel.bind(this));
                        this.getCharacteristic(batService, Characteristic.StatusLowBattery).on('get', this.getLowBatteryStatus.bind(this));
                        //this.getCharacteristic(batService, Characteristic.ChargingState).on('get', this.getChargingState.bind(this));
                        this.services.push(batService);
                        break;                    
                    } else {
            ...
            case this.Type == "Temp" || this.Type == "Temp + Humidity" || this.Type == "Temp + Humidity + Baro":
                {
                    ...
                    if (this.batteryRef && this.batteryRef !== 255) {
                        ...
                        this.gracefullyAddCharacteristic(temperatureSensorService, Characteristic.BatteryLevel).on('get', this.getBatteryLevel.bind(this));
                    }
            ...
            case this.Type == "Air Quality":
                {
                    ...
                    airParticulateDensityCharacteristic.setProps({
                        maxValue: 3000
                    });
